...
export async function getPopularPolls(
	...
	const response = await fetch(
		`${API_URL}/polls/popular?${params.toString()}`,
		{
		...
		return {
			todayVotes: Number(
				data.votesToday ??
					...
					data.votes_today ??
					data.today_votes ??
					fallbackStats.todayVotes,
			),
			activeUsers: Number(
				data.activeUsers ?? data.active_users ?? fallbackStats.activeUsers,
			),
			engagementPercentage: Number(
				data.engagementRate ??
					data.engagementPercentage ??
					data.engagement_rate ??
					data.engagement_percentage ??
					fallbackStats.engagementPercentage,
			),
			totalVotes: Number(
				data.registeredVotes ??
					data.totalVotes ??
					data.registered_votes ??
					data.total_votes ??
					fallbackStats.totalVotes,
			),
			totalDiscussions: Number(
				data.discussionsCreated ??
					data.totalDiscussions ??
					data.discussions_created ??
					data.total_discussions ??
					fallbackStats.totalDiscussions,
			),
		};
	} catch {
